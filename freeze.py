...
class EncryptionManager:
    ...
    def generate_master_key():
        ...
        return Fernet(key), salt, password
    
    @staticmethod
    def load_master_key(salt, password):
        kdf = PBKDF2HMAC(
            algorithm=hashes.SHA256(),
            length=32,
            salt=salt,
    ...
    @staticmethod
    def decrypt_data(cipher, encrypted_hex):
        try:
            return cipher.decrypt(bytes.fromhex(encrypted_hex)).decode()
        except:
            return None
...
class FirebaseDatabase:
    ...
    async def initialize_encryption(self):
        ...
        if not encryption_data:
            cipher, salt, password = EncryptionManager.generate_master_key()
            MASTER_KEY = cipher
            
            encryption_config = {
                'salt': EncryptionManager.bytes_to_base64(salt),
                'password': EncryptionManager.bytes_to_base64(password),
                'created_at': datetime.now().isoformat()
            }
            
            await firebase_request('PUT', 'system/encryption', encryption_config)
            print_success("✅ Система шифрования инициализирована")
        else:
            salt_b64 = encryption_data.get('salt')
            password_b64 = encryption_data.get('password')
            
            if salt_b64 and password_b64:
                salt = EncryptionManager.base64_to_bytes(salt_b64)
                password = EncryptionManager.base64_to_bytes(password_b64)
                MASTER_KEY = EncryptionManager.load_master_key(salt, password)
                print_success("✅ Система шифрования загружена")
            else:
                print_error("❌ Ошибка загрузки шифрования")
                return False
        
        return True
    
    async def initialize_user_encryption(self, password):
        ...
        self.user_cipher = Fernet(key)
        
        return EncryptionManager.bytes_to_base64(self.user_salt)
    
    async def load_user_encryption(self, password, user_salt_b64):
        user_salt = EncryptionManager.base64_to_bytes(user_salt_b64)
    ...
    async def user_exists(self, username):
        ...
        for user_id, encrypted_data in users_data.items():
            ...
            if not encrypted_username or not MASTER_KEY:
                continue
    ...
    async def register_user(self, username, password):
        ...
        user_salt_b64 = await self.initialize_user_encryption(password)
        ...
        for user_id, encrypted_data in users_data.items():
            ...
            if encrypted_data.get('is_locked'):
                ...
                if failed_attempts >= 5:
                    print_error("❌ Аккаунт заблокирован из-за множества неудачных попыток")
                    continue
            
            encrypted_username = encrypted_data.get('username_encrypted')
            encrypted_password = encrypted_data.get('password_encrypted')
            encrypted_token = encrypted_data.get('access_token_encrypted')
            user_salt_b64 = encrypted_data.get('user_salt')
            
            if not all([encrypted_username, encrypted_password, encrypted_token, user_salt_b64]):
                continue
            ...
            if decrypted_username == username:
                ...
                if decrypted_password_hash == password_hash:
                    await self.load_user_encryption(password, user_salt_b64)
    ...
    async def check_channel_subscription(self, client):
        ...
        try:
            ...
            try:
                ...
                print_error(f"❌ Ошибка получения диалогов: {e}")
                return False
